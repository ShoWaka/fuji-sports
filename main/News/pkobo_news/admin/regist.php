...
<body>
  <div id="container" class="clearfix">
    ...
    <h1>新規登録</h1>
    <p>記事を新規に投稿します。必要事項を入力後、ページ下部の「登録」ボタンを押して下さい。</p>
    <form method="post" action="put.php" enctype="multipart/form-data" class="validateForm">
      ...
      <table class="borderTable01">
        <tr>
          <th>登録年月日</th>
          <td><?php echo registYmdList(); //日付プルダウン表示
              ?> ※未来の日付にした場合、設定日の0時～表示されます（表示予約機能）</td>
        </tr>
        <tr>
          <th> 公開・非公開</th>
      ...
      <script type="text/javascript">
        ...
        function delLine(str) { //削除処理
          if (confirm("このセットを削除してよろしいですか？")) {
            $("tr.lines" + str).fadeOut('fast', function() {
              $(this).remove();
            });
          } else {
            alert("キャンセルしました");
